...
static sedi_uart_reg_t *sedi_uart[SEDI_UART_NUM] = {
	(sedi_uart_reg_t *)SEDI_UART_0_BASE, (sedi_uart_reg_t *)SEDI_UART_1_BASE,
	(sedi_uart_reg_t *)SEDI_UART_2_BASE, (sedi_uart_reg_t *)SEDI_UART_3_BASE,
	(sedi_uart_reg_t *)SEDI_UART_4_BASE, (sedi_uart_reg_t *)SEDI_UART_5_BASE
};

static volatile sedi_uart_context_t uart_context[SEDI_UART_NUM];
...
static uint32_t uart_dma_hs_id[SEDI_UART_NUM] = {
	DMA_HWID_UART0, DMA_HWID_UART1, DMA_HWID_UART2,
	DMA_HWID_UART3, DMA_HWID_UART4, DMA_HWID_UART5
};
...
static uart_dma_ctxt_t dma_write_ctxt[SEDI_UART_NUM] = {
	...
	{ .dma_xfer = NULL, .uart = SEDI_UART_3, .operation = WRITE },

	{ .dma_xfer = NULL, .uart = SEDI_UART_4, .operation = WRITE },

	{ .dma_xfer = NULL, .uart = SEDI_UART_5, .operation = WRITE }
};
...
static uart_dma_ctxt_t dma_read_ctxt[SEDI_UART_NUM] = {
	...
	{ .dma_xfer = NULL, .uart = SEDI_UART_3, .operation = READ },

	{ .dma_xfer = NULL, .uart = SEDI_UART_4, .operation = READ },

	{ .dma_xfer = NULL, .uart = SEDI_UART_5, .operation = READ }
};
...
{
	static bool uart_rst_done = false;
...
int sedi_uart_set_config(IN sedi_uart_t uart, IN sedi_uart_config_t *cfg)
{
	...
	ret = sedi_uart_set_baud_rate(uart, cfg->baud_rate, cfg->clk_speed_hz);
...
static void sedi_uart_save_context(IN sedi_uart_t uart)
{
	...
	volatile sedi_uart_context_t *ctx = &uart_context[uart];
			      ...
			      IN uint32_t length, OUT uint32_t *status)
{
	...
	ret =
		sedi_uart_dma_io_polled(uart, dma_dev, channel, buff, length, READ);
...
int32_t sedi_uart_set_power(IN sedi_uart_t uart, IN sedi_power_state_t state)
{
	...
	driver_id_t id = DRIVER_ID_UART0 + uart;
